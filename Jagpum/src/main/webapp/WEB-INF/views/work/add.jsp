...
$(function(){
	const status={
		...
		workContract:false,
		ok:function(){
		return this.workName&& this.workGenre&&
		this.workPrefer&&this.workPaid&&this.workContract;
	},
};
	$("[name=workName]").blur(function(){
		const size=$(this).val().length>0;
		$(this).removeClass("is-valid is-invalid")
		.addClass(size ? "is-valid":"is-invalid");
		status.workName=size;
	});
	$("[name=workGenre]").blur(function(){
	...
	$("[name=workPrefer]").blur(function(){
		const size=$(this).val().length>0;
		$(this).removeClass("is-valid is-invalid")
		.addClass(size?"is-valid":"is-invalid");
		status.workPrefer=size;
	});
	$("[name=workPaid]").blur(function(){
		const size=$(this).val().length>0;
		$(this).removeClass("is-valid is-invalid")
		.addClass(size?"is-valid":"is-invalid");
		status.workPaid=size;
	});
	$("[name=workContract]").blur(function(){
		const size=$(this).val().length>0;
		$(this).removeClass("is-valid is-invalid")
		.addClass(size?"is-valid":"is-invalid");
...
<div class="container drag-prevent">
	...
	<div class="row mt-4">
		...
		<div class="row mt-4">
			<div class="col-sm-12">
				<textarea  name="workSubtotal" class="form-control" placeholder="작품명 설명을 입력하세요"
...
<!-- 		<div class="row mt-4">
			<div class="col-sm-12 mb-3">
				<input class="form-check-input size-r" type="radio" name="workBook" value="Y">
					책
				<span>
				<input class="form-check-input size-r ms-3" type="radio" name=workBook value="N">
				책아님
				</span>
			</div>
		</div> -->
